...
public class DisplaymodCommand extends Command{
    ...
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Finds one module Information based on given " +
            "parameters. "
            + "Parameters: "
    ...
    public static String generateResultString(Model model){
        ...
        for(int i = 0; i < model.getDisplayList().size(); i++){
            result = result + "\n \n" +String.format(MESSAGE_SUCCESS, model.getDisplayList().get(i).getCode(),
